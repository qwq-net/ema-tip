...
interface RaceBetsProps {
  overview: Awaited<ReturnType<typeof getRaceBetOverview>>;
  page: Awaited<ReturnType<typeof getRaceBetGroupPage>>;
  params: BetGroupListParams;
  /** 絞り込みと並び替えのリンク先。クエリを外したレース馬券一覧の URL を渡す。 */
  basePath: string;
}

/**
 * レースに購入された馬券の一覧。購入 1 回を 1 行にし、検索・絞り込み・並び替え・ページ送りは
 * すべて URL クエリで表す。件数超過のページ送り直しは呼び手が済ませている前提。
 */
export function RaceBets({ overview, page, params, basePath }: RaceBetsProps) {
  const hasFilter = params.q !== undefined || params.type !== undefined || params.status !== undefined;
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterForm params={params} basePath={basePath} />

      <TableShell>
        <TableHead>
          <SortHeader label="購入日時" sortKey="createdAt" params={params} basePath={basePath} />
          <Th>ユーザー</Th>
          <Th>券種</Th>
          <Th>買い目</Th>
          <Th className="text-right">点数</Th>
          <SortHeader label="投票額" sortKey="amount" params={params} basePath={basePath} className="text-right" />
          <SortHeader label="払戻" sortKey="payout" params={params} basePath={basePath} className="text-right" />
          <Th>状態</Th>
        </TableHead>
        <TableBody>
          {page.rows.length === 0 && (
      ...
      <Pagination params={params} total={page.total} basePath={basePath} />
    </div>
  );
}
